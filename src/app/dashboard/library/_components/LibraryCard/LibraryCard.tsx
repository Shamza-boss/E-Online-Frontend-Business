...
function LibraryCard({ file, onRead }: LibraryCardProps) {
  ...
  return (
    <StyledCard
      ...
      sx={{
        borderWidth: 1.5,
        borderColor: alpha(LIBRARY_ACCENT, 0.15),
        borderRadius: 3,
        cursor: 'pointer',
        transition: 'all 0.25s cubic-bezier(0.4, 0, 0.2, 1)',
        '&:hover': {
          borderColor: alpha(LIBRARY_ACCENT, 0.4),
          backgroundColor: alpha(LIBRARY_ACCENT, 0.02),
          boxShadow: `0 4px 20px ${alpha(LIBRARY_ACCENT, 0.1)}`,
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <Box
          ...
          sx={{
            ...
            borderColor: alpha(LIBRARY_ACCENT, 0.1),
            ...
            background:
              isLoading || !thumbnail
                ? `linear-gradient(135deg, ${alpha(LIBRARY_ACCENT, 0.05)} 0%, ${alpha('#8B5CF6', 0.05)} 100%)`
                : 'transparent',
          }}
        >
          {isLoading || !thumbnail ? (
            <>
              <Skeleton
                ...
                sx={{
                  ...
                  top: 0,
                  left: 0,
                  bgcolor: alpha(LIBRARY_ACCENT, 0.08),
                }}
                animation="wave"
              />
              <Box
              ...
              >
                <Box
                  sx={{
                    p: 2,
                    borderRadius: '50%',
                    background: alpha(LIBRARY_ACCENT, 0.1),
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                  }}
                >
                  <PictureAsPdfIcon
                    sx={{ fontSize: 40, color: alpha(LIBRARY_ACCENT, 0.4) }}
                  />
                </Box>
              </Box>
            </>
        ...
        <Box
          sx={{
            ...
            borderTop: '1px solid',
            borderColor: alpha(LIBRARY_ACCENT, 0.08),
          }}
        >
          <Tooltip
            title={
