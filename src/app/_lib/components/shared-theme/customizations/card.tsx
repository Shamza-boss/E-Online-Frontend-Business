...
type ClassCardProps = {
  ...
  /** When set, the card navigates in-app (same tab) via Next.js Link. */
  href?: Route;
};
    ...
    >
      {href ? (
        <CardActionArea
          component={Link}
          href={href}
          sx={{ borderRadius: 2.5, textDecoration: 'none', color: 'inherit' }}
        >
          {actionArea}
        </CardActionArea>
      ) : (
        <CardActionArea sx={{ borderRadius: 2.5 }}>{actionArea}</CardActionArea>
      )}
    </StyledCard>
  );
}
