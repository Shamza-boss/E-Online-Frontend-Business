...
export const PdfNoteLink = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    const encodedPayload = attrs['data-pdf-payload'];
    const sentinelText =
      typeof encodedPayload === 'string'
        ? buildPdfNoteSentinelText(encodedPayload)
        : null;

    const sentinelNode = sentinelText
      ? [
          'span',
          {
            ...
            hidden: 'hidden',
            style:
              'display:none!important;visibility:hidden;width:0;height:0;overflow:hidden;padding:0;margin:0;',
          },
          sentinelText,
        ]
      : null;
